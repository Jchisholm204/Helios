...
class SnoutNetVisualizer:
    ...
    def visualize_predictions(self, test_loader, test_dataset, num_samples=12, save_path=None):
        ...
        with torch.no_grad():
            for idx in sample_indices:
                ...
                # Scale coordinates to 227x227 coordinate system
                filename = test_dataset.labels.iloc[idx]['filename']
                ...
                # Calculate distance using scaled coordinates
                distance = self.calculate_distance(pred_coord, true_coord_scaled)
                
                # Store results
                predictions.append(pred_coord)
                ground_truths.append(true_coord_scaled)
                images.append(image)
    ...
    def _create_visualization_grid(self, images, predictions, ground_truths, distances, 
        ...
        for i in range(num_samples):
            ...
            gt_circle = patches.Circle((gt_x, gt_y), 8, linewidth=2, 
            ...
            ax.plot(pred_x, pred_y, 'r+', markersize=12, markeredgewidth=3, label='Prediction')
            
            # Draw line connecting prediction to ground truth
            ax.plot([pred_x, gt_x], [pred_y, gt_y], 'b--', alpha=0.7, linewidth=1)
    ...
    def visualize_single_prediction(self, test_dataset, sample_idx, save_path=None):
        ...
        # Ground truth
        gt_circle = patches.Circle((true_coord_scaled[0], true_coord_scaled[1]), 10, 
                                 linewidth=3, edgecolor='green', facecolor='none')
        ax1.add_patch(gt_circle)
        
        # Prediction
        ax1.plot(pred_coord[0], pred_coord[1], 'r+', markersize=15, markeredgewidth=4)
        
        # Error line
        ax1.plot([pred_coord[0], true_coord_scaled[0]], [pred_coord[1], true_coord_scaled[1]], 
        ...
        # Zoomed view around the nose area
        margin = 50
        x_min = int(max(0, min(pred_coord[0], true_coord_scaled[0]) - margin))
        x_max = int(min(227, max(pred_coord[0], true_coord_scaled[0]) + margin))
        y_min = int(max(0, min(pred_coord[1], true_coord_scaled[1]) - margin))
        y_max = int(min(227, max(pred_coord[1], true_coord_scaled[1]) + margin))
        
        ax2.imshow(image_np[y_min:y_max, x_min:x_max])
        
        # Adjust coordinates for zoomed view
        gt_circle_zoom = patches.Circle((true_coord_scaled[0] - x_min, true_coord_scaled[1] - y_min), 8, 
                                      linewidth=2, edgecolor='green', facecolor='none')
        ax2.add_patch(gt_circle_zoom)
        
        ax2.plot(pred_coord[0] - x_min, pred_coord[1] - y_min, 'r+', markersize=12, markeredgewidth=3)
        ax2.plot([pred_coord[0] - x_min, true_coord_scaled[0] - x_min], 
                [pred_coord[1] - y_min, true_coord_scaled[1] - y_min], 'b--', alpha=0.8, linewidth=2)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Visualize SnoutNet predictions')
    ...
    if args.single is not None:
        ...
        if args.single >= len(test_dataset):
            print(f"Error: Sample index {args.single} out of range (0-{len(test_dataset)-1})")
            return
        
        save_path = args.save or f"single_prediction_{args.single}.png"
        visualizer.visualize_single_prediction(test_dataset, args.single, save_path)
    else:
        # Visualize multiple samples
        save_path = args.save or f"predictions_grid_{os.path.basename(args.model_path).split('.')[0]}.png"
