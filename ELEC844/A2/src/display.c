/**
 * @file display.c
 * @author Jacob Chisholm (https://Jchisholm204.github.io)
 * @brief
 * @version 0.1
 * @date Created: 2025-10-08
 * @modified Last Modified: 2025-10-08
 *
 * @copyright Copyright (c) 2025
 */

#include "display.h"

#include <SDL2/SDL_ttf.h>

#define PIXEL_PER_GRID 10
#define IN_PIXELS(x) (PIXEL_PER_GRID * x)
#define PX_BORDER 40
#define ARROW_SIZE 8

#define FONT "/usr/share/fonts/dejavu-sans-fonts/DejaVuSans.ttf"

void draw_text(SDL_Renderer* renderer, TTF_Font* font, const char* text, int x,
               int y);
void draw_arrow(SDL_Renderer* ren, int x1, int y1, int x2, int y2, int size);

disp_t* disp_init(size_t disp_size) {
    // Create the Display Object
    disp_t* pDisp = malloc(sizeof(disp_t));
    if (!pDisp)
        goto alloc_failure;

    pDisp->d_size = disp_size;

    if (SDL_Init(SDL_INIT_VIDEO) != 0)
        goto sdl_failure;
    if (TTF_Init() != 0)
        goto sdl_failure;

    // Create the SDL window
    pDisp->sdl_win = SDL_CreateWindow("844A1", 100, 100,
                                      IN_PIXELS(disp_size) + PX_BORDER * 2,
                                      IN_PIXELS(disp_size) + PX_BORDER * 2,
                                      SDL_WINDOW_SHOWN);
    if (!pDisp->sdl_win)
        goto win_failure;

    // Create the SDL renderer
    pDisp->sdl_ren =
        SDL_CreateRenderer(pDisp->sdl_win, -1, SDL_RENDERER_ACCELERATED);
    if (!pDisp->sdl_ren) {
        goto ren_failure;
    }

    pDisp->sdl_font = TTF_OpenFont(FONT, 16);
    if (!pDisp->sdl_font) {
        printf("TTF Openfont Error: %s\n", TTF_GetError());
        goto ren_failure;
    }
    return pDisp;

ren_failure:
    SDL_DestroyWindow(pDisp->sdl_win);
win_failure:
    SDL_Quit();
sdl_failure:
    free(pDisp);
alloc_failure:
    return NULL;
}

void disp_clr(disp_t* pDisplay) {
    // Clear the screen
    SDL_SetRenderDrawColor(pDisplay->sdl_ren, 255, 255, 255, 255);
    SDL_RenderClear(pDisplay->sdl_ren);
}

void disp_drawGrid(disp_t* pDisplay) {
    if (!pDisplay)
        return;

    // Render the drawing box
    SDL_SetRenderDrawColor(pDisplay->sdl_ren, 0, 0, 0, 255);
    // Left line
    SDL_RenderDrawLine(pDisplay->sdl_ren,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER, PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER);
    // Left Line
    SDL_RenderDrawLine(pDisplay->sdl_ren, PX_BORDER, PX_BORDER, PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER);
    // Top Line
    SDL_RenderDrawLine(pDisplay->sdl_ren, PX_BORDER, PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER, PX_BORDER);
    // Bottom Line
    SDL_RenderDrawLine(pDisplay->sdl_ren, PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER,
                       IN_PIXELS(pDisplay->d_size) + PX_BORDER);

    // Render text on bottom of screen

    draw_text(pDisplay->sdl_ren, pDisplay->sdl_font,
              "ELEC 844 2D Search Simulator - Jacob Chisholm",
              IN_PIXELS(pDisplay->d_size) / 4 + PX_BORDER,
              IN_PIXELS(pDisplay->d_size) + PX_BORDER * 1.2);

    // Call disp_render to render the frame
}

void disp_drawPoints(disp_t* pDisplay, struct spacial* pSpace) {
    if (!pDisplay)
        return;
    if (!pSpace)
        return;
    if (!pSpace->blocks)
        return;
    size_t grid_size = pSpace->dim.x * pSpace->dim.y;
    for (int i = 0; i < pSpace->n_voxels; i++) {
        struct voxel *v = &pSpace->voxels[i];
        if(v->state == eStateEmpty) continue;
        if(v->state == eStateBlocked)
            SDL_SetRenderDrawColor(pDisplay->sdl_ren, 255, 0, 0, 255);
        else
            SDL_SetRenderDrawColor(pDisplay->sdl_ren, 0, 0, 0, 255);
        SDL_Rect r = {IN_PIXELS(v->x) + PX_BORDER, IN_PIXELS(v->y) + PX_BORDER,
                      PIXEL_PER_GRID, PIXEL_PER_GRID};
        SDL_RenderFillRect(pDisplay->sdl_ren, &r);
    }
}

void disp_render(disp_t* pDisplay) {
    // Render the output
    SDL_RenderPresent(pDisplay->sdl_ren);
}

void disp_exit(disp_t** ppDisplay) {
    if (!ppDisplay)
        return;
    if (!(*ppDisplay))
        return;

    // Shutdown SDL
    SDL_DestroyRenderer((*ppDisplay)->sdl_ren);
    SDL_DestroyWindow((*ppDisplay)->sdl_win);
    SDL_Quit();

    // Free Display Data Structures
    free(*ppDisplay);
    // Set userspace reference to NULL
    *ppDisplay = NULL;
}

void draw_text(SDL_Renderer* renderer, TTF_Font* font, const char* text, int x,
               int y) {
    SDL_Color color = {0, 0, 0, 255}; // white text

    SDL_Surface* surface = TTF_RenderText_Solid(font, text, color);
    if (!surface) {
        return;
    }

    SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_FreeSurface(surface);
    if (!texture) {
        fprintf(stderr, "CreateTexture Error: %s\n", SDL_GetError());
        return;
    }

    SDL_Rect dst = {x, y, 0, 0};
    SDL_QueryTexture(texture, NULL, NULL, &dst.w, &dst.h);
    SDL_RenderCopy(renderer, texture, NULL, &dst);
    SDL_DestroyTexture(texture);
}

void draw_arrow(SDL_Renderer* ren, int x1, int y1, int x2, int y2, int size) {
    // Draw main line
    SDL_RenderDrawLine(ren, x1, y1, x2, y2);

    // Calculate the arrowhead angle
    double angle = atan2(y2 - y1, x2 - x1);
    double arrow_angle = M_PI / 6; // 30 degrees
    double arrow_length = size;

    // Left side of arrowhead
    int x3 = x2 - arrow_length * cos(angle - arrow_angle);
    int y3 = y2 - arrow_length * sin(angle - arrow_angle);
    SDL_RenderDrawLine(ren, x2, y2, x3, y3);

    // Right side of arrowhead
    int x4 = x2 - arrow_length * cos(angle + arrow_angle);
    int y4 = y2 - arrow_length * sin(angle + arrow_angle);
    SDL_RenderDrawLine(ren, x2, y2, x4, y4);
}
